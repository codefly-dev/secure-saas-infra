...
new PolicyPack("secure-saas-infra-guardrails", {
  ...
  policies: [
    ...
    },
    {
      ...
      severity: "critical",
      validateResource: (args, reportViolation) => {
        if (args.type !== "aws:ec2/securityGroup:SecurityGroup") return;

        for (const rule of args.props.ingress ?? []) {
          if (
    ...
    },
    {
      ...
      validateResource: (args, reportViolation) => {
        ...
        if (
          ...
          reportViolation(
            "VPC Flow Logs must deliver to CloudWatch Logs or S3.",
          );
        }
      },
    },
    ...
    },
    {
      ...
      validateResource: (args, reportViolation) => {
        if (isIamPolicyDocumentResource(args)) {
          for (const violation of iamPolicyDocumentViolations(args.props.policy)) {
        ...
        if (isHighRiskManagedPolicyArn(policyArn)) {
          reportViolation(
            "IAM roles, users, and groups must not attach AdministratorAccess, PowerUserAccess, or IAMFullAccess. Use reviewed least-privilege policies.",
          );
        }
      },
    },
    ...
    },
    {
      ...
      validateResource: (args, reportViolation) => {
        ...
        if (rules.length === 0) {
          reportViolation("S3 buckets must declare server-side encryption rules.");
          return;
        }
    ...
    {
      name: "rds-proxies-require-tls",
      description: "RDS Proxies must require TLS to defeat in-transit eavesdropping.",
      severity: "high",
      validateResource: (args, reportViolation) => {
    ...
    },
    {
      ...
      description: "AWS Backup vaults must encrypt with a customer-managed KMS key.",
      severity: "critical",
      validateResource: (args, reportViolation) => {
...
function isProtectedBucket(args: ResourceValidationArgs) {
  return (
    args.name.includes("audit-logs") || args.name.includes("vault-backups")
  );
}
...
function hasPrincipalAccountCondition(policy: unknown) {
  ...
  try {
    ...
    return statements.some((statement) => {
      ...
      return Object.values(condition).some((conditionValues) =>
...
function iamPolicyDocumentViolations(policy: unknown) {
  ...
  try {
    ...
    return ["IAM policy documents must be valid JSON so guardrails can inspect them."];
  }
}

function normalizeStatements(statement: unknown): Array<Record<string, unknown>> {
...
function hasPrivilegeEscalationAction(action: unknown): boolean {
  ...
  if (typeof action === "string") {
    ...
    return highRiskActions.some((candidate) => action.toLowerCase() === candidate.toLowerCase());
  }
