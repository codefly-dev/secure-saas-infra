...
test("security tooling enables organization-wide detection and aggregation", async () => {
  ...
  assert.equal(
    resourcesOfType(resources, "aws:guardduty/detector:Detector").length,
    1,
  );
  assert.equal(
    resourcesOfType(
  ...
  assert.equal(
    resourcesOfType(resources, "aws:securityhub/account:Account").length,
    1,
  );
  assert.equal(
    resourcesOfType(
  ...
  assert.equal(
    ...
    )[0].inputs.resourceTypes.includes("LAMBDA_CODE"),
    true,
  );
  assert.equal(
    resourcesOfType(
    ...
    )[0].inputs.organizationAggregationSource.allRegions,
    true,
  );
});
