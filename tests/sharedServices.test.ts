...
test("shared services bootstraps Vault KMS, backup storage, and Tailscale secrets", async () => {
  ...
  assert.deepEqual(secrets.map((secret) => secret.inputs.name).sort(), [
    "deus-unit-tailscale-oauth-client-id",
    "deus-unit-tailscale-oauth-client-secret",
  ]);
});
